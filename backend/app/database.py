...
# Import correcto (usando la instancia que creamos en config.py)
from app.core.config import settings


def _should_require_ssl(database_url: str) -> bool:
    """Determina si se debe forzar SSL (necesario en Render)"""
...
def _get_connect_args(database_url: str) -> dict[str, str]:
    """Agrega sslmode=require para bases de datos externas como Render"""
    if (
        settings.DATABASE_URL 
        and _should_require_ssl(database_url) 
...
try:
    ...
    if not database_url:
        raise ValueError(
            "Database configuration is missing. Set DATABASE_URL environment variable."
        )
